...
{
    IDataView dataView = mlContext.Data.LoadFromTextFile<TaxiTrip>(_trainDataPath, ',', true);
    var pipeline = mlContext.Transforms.CopyColumns(outputColumnName: "Label", inputColumnName: "FareAmount")
        // 这个例子中，这三个特征转换处理除了使用独热编码，使用特征化文本也是一模一样的效果（理论上，特征化文本适用范围更广）
        ...
        //.Append(mlContext.Transforms.Text.FeaturizeText("VendorIdEncoded", "VendorId"))
        //.Append(mlContext.Transforms.Text.FeaturizeText("RateCodeEncoded", "RateCode"))
        //.Append(mlContext.Transforms.Text.FeaturizeText("PaymentTypeEncoded", "PaymentType"))
...
{
    IDataView dataView = mlContext.Data.LoadFromTextFile<TaxiTrip>(_testDataPath, ',', true);
