...
string _assetsPath = Path.Combine(Environment.CurrentDirectory, "../../../Data");
...
ClassifyBatchImage(mlContext, model);
...
void ClassifyBatchImage(MLContext mlContext, ITransformer model)
{
    var predictor = mlContext.Model.CreatePredictionEngine<ImageData, ImagePrediction>(model);

    var ps = Directory.GetFiles(_imagesFolder);
    var ids = ps.Where(it=>Path.GetExtension(it)==".jpg"||Path.GetExtension(it)==".png")
        .Select(it => new ImageData { ImagePath = it }).ToList();
    ids.ForEach(it =>
    {
        // Make prediction function (input = ImageData, output = ImagePrediction)
        var prediction = predictor.Predict(it);
        Console.WriteLine($"Image: {Path.GetFileName(it.ImagePath)} predicted as: {prediction.PredictedLabelValue} with score: {prediction.Score.Max()} ");
    });
}
